...
schedules_df <- purrr::map_dfr(1:length(years_vec), function(x){
  season_pull <- years_vec[[x]]
  proxy <- select_proxy(proxies)
  
  completed_sched <- wehoop::wnba_leaguegamefinder(season = years_vec[[x]], proxy = proxy)$LeagueGameFinderResults %>% 
    rejoin_schedules() %>% 
    dplyr::mutate(
      Season = years_vec[[x]])
  
  playoff_sched <- data.frame()
  playoff_sched <- wehoop::wnba_leaguegamefinder(season = years_vec[[x]], season_type = "Playoffs", proxy = proxy)$LeagueGameFinderResults %>% 
    rejoin_schedules() %>% 
    dplyr::mutate(
      Season = years_vec[[x]])
...
#--   rebuild master schedule files ----
#
sched_list <- list.files(path = glue::glue('wnba_stats/schedules/csv'))

master_schedules_df <- purrr::map_dfr(sched_list, function(x){
  sched <- data.table::fread(paste0('wnba_stats/schedules/csv/',x))
  return(sched)
})
...
schedules_df <- data.table::fread("wnba_stats_schedule_master.csv")

player_logs <- wehoop::wnba_leaguegamelog(season = y, player_or_team = "P", proxy = proxy) %>%  # change season here
  purrr::pluck("LeagueGameLog") %>%
  janitor::clean_names() %>%
  dplyr::mutate(team_location = ifelse(stringr::str_detect(matchup, "\\@"), "away", "home"),
                dplyr::across(c("player_id", "team_id"), as.integer))

jsonlite::write_json(player_logs, glue::glue("wnba_stats/json/player_game_logs/player_game_logs_{y}.json"))
...
suppressWarnings(
  wnba_pbp_df_raw <- wnba_pbp_df_raw %>%
    dplyr::mutate(dplyr::across(c("player1_id", "player2_id", "player3_id", "player1_team_id", "player2_team_id", "player3_team_id", "game_id"), as.integer))
)
wnba_pbp_df <- wnba_pbp_df_raw %>%
  dplyr::left_join(player_logs %>%
                     dplyr::distinct(player1_id = player_id, player1_name = player_name),
                   by = "player1_id") %>%
  dplyr::left_join(player_logs %>%
                     dplyr::distinct(player2_id = player_id, player2_name = player_name),
                   by = "player2_id") %>%
  dplyr::left_join(player_logs %>%
                     dplyr::distinct(player3_id = player_id, player3_name = player_name),
                   by = "player3_id") %>%
  dplyr::left_join(player_logs %>%
...
missing_st_qtr <- function(game_id_miss, period_miss){
  wehoop::wnba_boxscoretraditionalv2(
    game_id = game_id_miss,
    start_period = period_miss,
    end_period = period_miss,
    range_type = 1,
    proxy = proxy) %>%
